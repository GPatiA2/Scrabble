...
public class MediumStrategy  extends RunStrategy<Maquina>{

	public MediumStrategy(String tipo) {
		super(tipo);
	}
	
	//Las maquinas de nivel medium intentaran colocar primero palabras de 4 letras, despues las de 3, las de 2 . . . 
	public List<Command> run(Game game, Turno t, List<Coordenadas> listaFichasNoFijas,
		...
		if (game.primeraPalabra()) {
			...
			if (!lista_palabras_posibles.isEmpty()) {
				...
				if (lista_palabras_posibles.containsKey(4)) {
			...
			while (indice_scoredWords >= 0 && !esPosibleColocar) {
				...
				while (indice_pivotes < pivotes.size() && !esPosibleColocar) {
					...
					char letra = pivotes.get(indice_pivotes).getKey();
