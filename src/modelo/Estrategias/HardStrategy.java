...
public class HardStrategy extends RunStrategy<Maquina>{

	public HardStrategy(String tipo) {
		super(tipo);
	}
	
	//Las maquinas Hard intentarán colocar primero las palabras de tamanio 6, despues las de 5 y despues las de 4 . . .
	//La primera palabra que colocan sera como maximo de 5 letras
	public List<Command> run(Game game, Turno t, List<Coordenadas> listaFichasNoFijas,
		...
		if (game.primeraPalabra()) {
			...
			if (!lista_palabras_posibles.isEmpty()) {
				...
				String palabra = "";
				if (lista_palabras_posibles.containsKey(5)) {
					palabra = lista_palabras_posibles.get(5).get(0);
				}
				else if (lista_palabras_posibles.containsKey(4)) {
				...
				else if (lista_palabras_posibles.containsKey(2)) {
			...
			while (indice_scoredWords >= 0 && !esPosibleColocar) {
				...
				//Obtenemos la lista de posibles pivotes para esta palabra (en este caso tanto vocales como consontantes)
				//El resultado es una lista con todas las letras de la palabra
				List<Pair<Character, Coordenadas>> pivotes = this.buscarPivotes(false, lista_palabras.get(indice_scoredWords));
